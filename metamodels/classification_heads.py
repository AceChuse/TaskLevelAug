...
from qpthhere.qp import QPFunction
...
class MetaOptNetHead_SVM_CS(nn.Module):
    def __init__(self, evalIter):
        super(MetaOptNetHead_SVM_CS, self).__init__()
        self.evalIter = evalIter

    def __call__(self, query, support, support_labels, n_way, n_shot, C_reg=0.1, double_precision=False):
        """
        Fits the support set with multi-class SVM and
        returns the classification score on the query set.
        ...
        #                 subject to Cz <= h
        # We use detach() to prevent backpropagation to fixed variables.
        qp_sol = QPFunction(verbose=False, maxIter=15 if self.training else self.evalIter)(G, e.detach(), C.detach(), h.detach(), A.detach(), b.detach())
        # qp_sol = QPFunction(verbose=True, maxIter=self.maxIter)(G, e.detach(), C.detach(), h.detach(), A.detach(), b.detach())
    ...
    def extra_repr(self):
        return 'evalIter={evalIter}'.format(**self.__dict__)
...
class ClassificationHead(nn.Module):
    def __init__(self, base_learner='MetaOptNet', enable_scale=True, evalIter=15):
        super(ClassificationHead, self).__init__()
        self.base_learner = base_learner
        if ('SVM-CS' in base_learner):
            self.head = MetaOptNetHead_SVM_CS(evalIter=evalIter)
    ...
    def extra_repr(self):
        return 'base_learner={base_learner}, enable_scale={enable_scale}'.format(**self.__dict__)
...
class Bagging(nn.Module):
    def __init__(self, base_learner, in_channels, n_channels, n_overlap, affine=True):
        ...
        assert(n_channels >= n_overlap)
        self.stride = n_channels - n_overlap
        self.n_learners = int((in_channels - n_channels) / self.stride) + 1
    ...
    def forward(self, query, support, support_labels, n_way, n_shot, **kwargs):
        ...
        support = support.view(tasks_per_batch * n_support, 1, self.in_channels, -1)
        feature_size = support.size(-1)
        support = F.unfold(support, (self.n_channels, feature_size), dilation=1, padding=0, stride=self.stride)
        support = support.permute(2, 0, 1).reshape(self.n_learners * tasks_per_batch, n_support, -1)

        query = query.view(-1, 1, self.in_channels, feature_size)
        query = F.unfold(query, (self.n_channels, feature_size), dilation=1, padding=0, stride=self.stride)
        query = query.permute(2, 0, 1).reshape(self.n_learners * tasks_per_batch, n_query, -1)

        support_labels = support_labels.repeat(self.n_learners, 1)

        logits = self.base_learner(query, support, support_labels, n_way, n_shot, **kwargs)
        logits = F.softmax(logits, dim=2).view(self.n_learners, tasks_per_batch, n_query, n_way).mean(dim=0)
    ...
    def extra_repr(self):
        return 'n_learners={n_learners}, n_channels={n_channels}, n_overlap={n_overlap}'.format(**self.__dict__)